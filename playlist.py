...
import struct, io
from PyQt5.QtGui import QStandardItem, QStandardItemModel, QBrush, QColor, QPalette
...
class PlaylistSet():
    ...
    def loadOldFormat(self, filepath):
        try:
            dat = io.StringIO(file(filepath, 'rb').read())
...
#        try:
            with zipfile.ZipFile(filepath, 'r') as zf:
                dat = pickle.loads(zf.read('playlist'))
    ...
    def save(self, filepath):
        with zipfile.ZipFile(filepath, 'w', zipfile.ZIP_DEFLATED) as zf:
            dat = [(l.Title, l.chooseMethod, l.playing, l.items) \
                for l in self.lists]
            zf.writestr('playlist', pickle.dumps(dat, -1))
