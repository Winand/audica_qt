...
from PyQt4.QtGui import QPainter, QBrush, QLinearGradient, QColor, QPalette, QWidget, QPen, QCursor
...
#        self.tip.move(gpos)
    ...
    def wheelEvent(self, e):
        vo, self.v = self.v, bound(0, self.v+(.03 if e.delta()>0 else -.03), 1)
        print vo
        if vo != self.v:
            self.userChanged.emit(self.v)
            self.changed.emit(self.v)
