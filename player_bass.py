...
class Player:
    ...
    def getStreamTags(self):
        with lock_tags:
            return tags.TAGS_Read(self.p.handle, "%UTF8(%ALBM\6%ARTI\6%CMNT\6%COMP\6%COPY\6%GNRE\6%TITL\6%TRCK\6%YEAR\6%SUBT\6%AART\6%DISC)").split('\6')
...
def init():
    ...
    for f in glob.iglob(u"bass?*.dll"):
        ...
        for i in range(pinfo.formatc):
            ...
            Plugin_list[plug][0][info.ctype] = info.name
            exts = info.exts.replace(";", " ")
            Filter.append(info.name + " (" + exts + ")")
            all_supp_exts += " " + exts     
