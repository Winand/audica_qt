...
from PyQt5.QtCore import QUrl, pyqtSignal, Qt, QTimer, QDir, QRect, QSize, QPoint, QEvent, QRegExp#, QTranslator
from PyQt5.QtGui import QPalette, QColor, QPainter, QImage, \
    QPixmap, QFont, QIcon, QCursor, QDesktopServices
from PyQt5.QtWidgets import QAbstractItemView, QListView, QGraphicsDropShadowEffect, \
    QApplication, QRubberBand, QMenu
...
class AuToolButton(QtWidgets.QToolButton):
...
class AuPlaylistView(QtWidgets.QTableView): #QTreeView
    resized = pyqtSignal(int, int)
    
    def __init__(self, *args, **kwargs):
        overmind(self).__init__(*args, **kwargs)
#!        self.header().setResizeMode(QtGui.QHeaderView.Fixed)
        self.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Fixed)  # https://doc.qt.io/archives/qt-5.11/qheaderview-obsolete.html#setResizeMode
...
class AuSpectrumBox(QtWidgets.QWidget):
    player = None
    ...
    def render(self):
        ...
        if self.mode == 0:
            ...
            for C in range(chans):
                ...
                v_buf = [int((1 - buf[x * chans + C]) * h / 2) for x in range(w)]
...
class AuFileDialog(QtWidgets.QFileDialog):
    def __init__(self, *args, **kwargs):
        print(args, kwargs)
        overmind(self).__init__(*args, **kwargs)
        QtWidgets.QPushButton(self)

class AuMainWindow(QtWidgets.QMainWindow):
    def __init__(self):
...
#        self.mnuExit.triggered.connect(self.closeEvent)
        ...
        self.tabs.setShape(QtWidgets.QTabBar.RoundedSouth)
        ...
        self.lbltags.setText(song_info_template.format( \
            song="Дожили", song_size=song_label_size, \
            artist="AC/DC", artist_search="Машина времени", \
...
#        filename = unicode()
    ...
    def filter_postponed(self):
        self.plv.model().setFilter(self.txtSearch.text())
...
#        QtCore.QCoreApplication.instance().quit()
    ...
    def opened(self, player, success):
        if not success:
            self.setWindowTitle("file cannot be opened Привет")
            return
